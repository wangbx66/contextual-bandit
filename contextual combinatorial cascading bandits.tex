...
\section{Problem Formulation-general case}
...
At time $t$, feature vectors $x_{t,a} \in \RR^{d \times 1}$ with $\norm{x_{t,a}}_2 \leq 1$ for every base arm $a \in E$ are revealed to the learning agent; the feature vectors combine both the information of the user and the corresponding base arm. \shengyu{Add a standard/typical example (just one sentence) to illustrate the combination?} For example, suppose the user at time $t$ is represented by the feature vector $u_t$ and the feature vector of the base arm $a$ is $v_a$, then we can let $x_{t,a}$ be $(u_t, v_a)$ which combines both the information of the user and the base arm. \shuai{I have added one sentence.} Then the learning agent recommends a feasible action $\bA_t=\{ \ba_{1}^t,...,\ba_{\abs{\bA_t}}^t \} \in \cS$ to the user and notice that the size of the action is not fixed. Then according to the user behavior and the structure of the action $\bA_t$, we observe the weights of fist $\bO_t$ base arms in $\bA_t$. For example in website recommender systems, if the user checks from the first recommended website and stops when she finds one interesting or has checked all websites, then the observed base arms are checked websites. 

Let $\cH_t$ denote the history before the learning agent chooses action at time $t$. Thus $\cH_t$ contains all information and observations at time $s < t$ and context information at time $t$. Given history $\cH_t$, we assume $\bw_{t,a}$'s are mutually independent and satisfy
\begin{equation} % eq:expectation
  \label{eq:expectation}
  \EE[\bw_{t,a} | \cH_t] = \theta_{\ast}^{\top} x_{t,a},
\end{equation}
...
We define random variable $\bO_t$ as the number of observed base arms in $\bA_t$. At the end of every time $t$, the agent observes the first $\bO_t$ items of $\bA_t$. We say that item $a$ is {\it observed} if $a = \ba_k^t$ for some $k \leq \bO_t$. Thus, $\cH_t$ consists of $\{x_s, \bA_s = (\ba_{1}^s,...,\ba_{\abs{\bA_s}}^s), \bO_s, \bw_s(\ba_k^s),x_t \mid  k \in[\bO_s], 1 \le s<t \}$.

We consider the position discount. For example in recommender systems, the positions to display advertisements might have difference prices, hence of different value when user clicks on it.

The reward function on round $t$ is an application dependent function. We assume the expected reward function $f(A, w)$ of an action $A$ with expected weights $w$ satisfy the following assumptions.

Monotonicity The expected reward $f(A, w)$ is a non-decreasing with respect to $w$. For any $d$ dimension vectors $w$ and $w'$, if $w(i) \leq w'(i)$, we have $f(A, w) \leq f(A, w')$.

Lipschitz continuity The expected reward function f(A, w) is a Lipschitz continuous with respect to $w$ together with position discount $\gamma$. In particular, there is a constant $B>0$ such that, for any $w$ and $w'$, we have
$$
	|f(A, w) - f(A, w')| \leq B\sqrt{\sum_{i \in Ind(A)} \gamma_i^2(w(i) - w'(i))^2}
$$

We don't require the agent knows how the function $f(A, w)$ is defined. We only assume the agent has access to an oracle $\cO(A, w)$. More precisely, an oracle $\cO(\cS, w)$ is  called an $\alpha$-approximation oracle for some $\alpha \leq 1$, if given input $w$ and $\cS$, the oracle always returns an action(super arm) $A = \cO(r, \cS) \in \cS$ satisfying $f(w, S) \geq \alpha opt_{w, \cS}$ where $opt_{w, \cS} = \max_{s \in \cS}$ f(r, S).

The $\alpha$-regret on time $t$ can be written as
$$
R_t = \alpha opt_{w_t} - f(w_t, A_t).
$$
Our goal is to minimize the cumulative regret $R = \sum_{t=1}^n R_t$.
...
\end{lemma}
\begin{proof}
	$w_{t, a} = \theta_*^{\top} x_{t,a}$. First by Holder's inequality,
	\begin{align*}
	...
	Then
	\begin{align*}
		\bU_t(a) - w_{t,a} &\leq \hat{\theta}_{t-1}^{\top}x_{t,a} + \bbeta_{t-1}(\delta)\norm{x_{t,a}}_{\bV_{t-1}^{-1}} - \theta_{\ast}^{\top}x_{t,a} \\
		&\leq 2 \bbeta_{t-1}(\delta)\norm{x_{t,a}}_{\bV_{t-1}^{-1}},
	\end{align*}
	and 
	\begin{align*}
		&\bU_t(a) - w_{t,a} \\
		&\qquad \geq \min\{\hat{\theta}_{t-1}^{\top}x_{t,a} + \bbeta_{t-1}(\delta)\norm{x_{t,a}}_{\bV_{t-1}^{-1}} - \theta_*^{\top} x_{t,a}, 1 - \theta_*^{\top} x_{t,a}  \} \\
		&\qquad \geq 0		
	\end{align*}
\end{proof}

Our proposed algorithm, C$^3$-UCB, is described in Algorithm \ref{alg:ConComCascade}. First, the learning agent computes the upper confidence bounds (UCBs) $\bU_t \in [0,1]^{E}$ on the expected weights of all base arms in $E$. Second, she uses the computed UCBs $\bU_t$ to select an action $\bA_t$. Third, she checks from the first base arm in $\bA_t$ and stops after checking the $\bO_t$-th base arm.
...
To state our main theorem, we need some definitions. Let $p_{t, A}$ be the {\em probability of full observation of $A$}, that is the probability that all base arms of $A = (a_1, \ldots, a_{\abs{A}})$ will be observed at time $t$. Let $p^* = \min_{1 \leq t \leq T} \min_{A \in \cS} ~ p_{t, A}$ to be the minimal probability that an action could have all base arms observed over all time. The following is the main theorem on the regret achieved by our C$^3$-UCB algorithm.

\begin{theorem} %main theorem
	\label{thm:main}
	Suppose the expected reward function, with parameters $\gamma_k, k\leq K$, is a function of expected weights of base arms and satisfies the requirements stated in section 3. Assume there is an $\alpha$-approximation oracle $\cO$. Then for any $\delta > 0$, with probability at least $1 - \delta$, the cumulative expected regrets of our algorithm, C$^3$-UCB, satisfy
	\begin{align}
...
\begin{proof} [of Theorem \ref{thm:main}] %proof outline of main theorem
	The complete proof is in the Appendix. The main idea is to reduce the regret at time $t$, $R(t, \bA_t)$, to the case that all items in $\bA_t$ have been observed. In particular,
	\begin{align*}
    	&\EE[R(t, \bA_t)|\cH_t] \\
    ...
    \end{align*}
    Also, by monotonicity and Lipschitz continuity,
    $$
    	R(t, \bA_t) \leq C \sqrt{\sum_{k=1}^{\bO_t} \bbeta_{t-1}(\delta) \| \gamma_k x_{t, \ba_k^t} \|_{\bV_{t-1}}^2}
    $$


	our analysis to that of CombCascade in \cite{kveton2015combinatorial}. This reduction is challenging for two reasons. First, our setting has contextual information. Second, we consider position-discounted weight $\gamma$.

	Our analysis can be trivially reduced to contextual combinatorial semi-bandits by conditioning on the event of observing all items. Then we can express the expected regret at time $t$ conditioned on the history $\cH_t$ as:
	\begin{align*}
    	&\EE[R(t, \bA_t)|\cH_t] \\
    ...
    % &\qquad \EE \left[ \left. R_{\wedge}(t, \bA_t) \EE \left[ \left. \frac{1}{p_{\wedge, t, \bA_t}} \bOne\{\bO_t\geq\abs{\bA_t}\} \right| \bA_t \right]  \right| \cH_t \right] \\
    \end{align*}
    and anaylize our problem under the assumption that all items in $\bA_t$ are observed. 
...
In the special case of single-choice-recommendations, the learning agent recommends a list of items to the user. Then the user checks from the first item and stops if she finds one interesting or has checked all items. Suppose we also consider position discount here. If the user interests in the $k$-th item, then the learning agent will receive reward $\gamma_k$. It is easy to check that in this case, the expected reward function is $f(A, w) = f(A,w) = \sum_{k = 1}^{\abs{A}} \gamma_{k} \prod_{i=1}^{k-1} (1 - w(a_i)) w(a_k)$. And if the parameters satisfy $1\geq \gamma_1 \geq \cdots \geq \gamma_K \geq 0$, then $f$ satisfies the requirements. (We leave all these verifications in appendix.) In this case, $p(t,A) p^* = $ Then we have
\begin{corollary}
	In the problem of single-choice-recommendations, the expected reward function is $f(A, w) = f(A,w) = \sum_{k = 1}^{\abs{A}} \gamma_{k} \prod_{i=1}^{k-1} (1 - w(a_i)) w(a_k)$. Then for any $\delta > 0$, with probability at least $1 - \delta$, the cumulative expected regrets of our algorithm, C$^3$-UCB, satisfy
	\begin{align}
		R(T) &\le \frac{\sqrt{2}}{p^*} \sqrt{TKd\ln(1 + C_\gamma T/(\lambda d))}  \nonumber \\
		&\qquad \cdot \left(\sqrt{d\ln \left( \frac{1 + C_\gamma T/(\lambda d)}{\delta^{2/d}}\right) } + \sqrt{\lambda}\right) \nonumber \\
		&=O\left(\frac{d}{p^*} \sqrt{TK} \ln \left(\frac{C_\gamma T}{\delta^{2/d}}\right) \right).
...
This reduction might be problematic because the probability $p_{\wedge, t, \bA_t}$ for the conjunctive case can be low.
...
The proof of this theorem is different in the following lemmas.

\subsection{Dissutions}

Our setting generalizes several existing works but not limited to them. Our setting generalizes cascading bandit and combinatorial bandit to involve position discount, 

to involve context

there are cases that can not be fit into their 

our method generalize Qin's method and has a regret bound proof.
...
\begin{figure*}
	...
	\subfigure[Disjunctive, $\gamma=1$]{
		\includegraphics[height = 3 cm, width = 4 cm]{synD-1.eps}}
	\subfigure[Disjunctive, $\gamma=0.95$]{
		\includegraphics[height = 3 cm, width = 4 cm]{synD-95.eps}}
	\subfigure[Conjunctive, $\gamma=1$ ]{
		\includegraphics[height = 3 cm, width = 4 cm]{synC-1.eps}}
	\subfigure[Conjunctive, network 1755, $\gamma=1$ ]{
		\includegraphics[height = 3 cm, width = 4 cm]{network3B-1.eps}}
...
\begin{figure*}
	\centering
	\subfigure[MovieLens]{
		\includegraphics[height = 3cm, width = 5 cm]{movie-1.eps}}
	\caption{MovieLens}

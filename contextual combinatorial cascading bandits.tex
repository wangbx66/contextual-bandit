...
\STATE {$\beta_{t}(\delta) \leftarrow \sqrt{\ln(\det(\bV_{t})) + 2 \ln(\frac{1}{\delta})} + \sqrt{\lambda}$ \wei{This $\beta$ is different from what is defined in Theorem 4.1.}}
\ENDFOR {~~$t$}
\end{algorithmic}
\end{algorithm}
...
\wei{The symbol here may need to be changed. $w_i$ coincides with weight $w_i$ before, 
	...
	about $p_i$.}
\shuai{Maybe we can replace $p_i$ by $w_i$ like the lemma above. I have changed accordingly.}
\begin{lemma}
...
&\qquad+ \gamma_{m+1}' (\prod_{k=1}^{K} u_k) (1 - u_{K+1})+ \prod_{k=1}^{K+1} u_k\\

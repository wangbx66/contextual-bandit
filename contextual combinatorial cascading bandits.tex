...
\STATE {$\beta_{t}(\delta) \leftarrow \sqrt{\ln(\det(\bV_{t})/(\lambda^d \delta^2))} + \sqrt{\lambda}$ }
\ENDFOR {~~$t$}
\end{algorithmic}
\end{algorithm}
...
\wei{$p_{\wedge, A}$ is not defined before. Only $p_{\wedge, t, A}$ is defined.
	I guess you are using $w$ vector to be the mean vectors for $A$. Please define
	this clearly.}
\shuai{I have modified this part.}
Suppose we have weights $w$. Let $p_{\wedge, A}$ be the {\em probability of full observation of $A$}, that is the probability that all base arms of $A = (a_1, \ldots, a_{\abs{A}})$ are observed under the conjunctive objective:
$$
...
$$
E[R(t, \bA_t)|\cH_t] \leq \frac{1}{p^*} \EE[ \beta_{t-1}(\delta) \sum_{k=1}^{\bO_t}\norm{\gamma_k x_{t,\ba_k^t}}_{\bV_{t-1}^{-1}} ].
$$
If we have
% $$
...
&E[R_{\wedge}(t, \bA_t)|\cH_t] \leq \frac{8}{f_{\wedge}^{\ast}} \EE[\beta_{t-1}(\delta)\sum_{k=1}^{\bO_t}\norm{\gamma_k x_{t,\ba_k^t}}_{\bV_{t-1}^{-1}}].
\end{align*}
\end{lemma}
\begin{proof}
...
Therefore
\begin{align*}
\EE_t[R_{\wedge}(t, \bA_t)] &\leq \EE[\frac{8}{f_{\wedge, t}^*} \bOne\{ \bO_t \geq \abs{\bB_t} \}\sum_{k=1}^{\abs{\bB_t}}\beta_{t-1}(\delta)\norm{\gamma_k x_{t,\ba_k^t}}_{\bV_{t-1}^{-1}}]\\
&\leq \EE[ \frac{8}{f_{\wedge}^*} \sum_{k=1}^{ \bO_t } \beta_{t-1}(\delta) \norm{\gamma_k x_{t,\ba_k^t}}_{\bV_{t-1}^{-1}} ]
\end{align*}
\end{proof}
...
$$
\sum_{s=1}^t \sum_{k=1}^{\bO_s} \norm{\gamma_k x_{s,\ba_{k}^s}}_{\bV_{s-1}^{-1}}^2 \leq 2\ln(\frac{\det(\bV_t)}{\lambda^d}).
$$
\end{lemma}
\begin{proof}
\begin{align*}

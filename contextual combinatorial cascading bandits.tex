...
We formulate the problem of {\em contextual combinatorial cascading bandit} as follows. Suppose we have a finite set  $E=\{1,...,L\}$ of $L$ \textit{ground items}, also referred to as {\em base arms}. Let $\cS$ be the set of \textit{feasible actions}. For any feasible action $A \in \cS$, $A$ has an underlying set, which is a subset of $E$, together with a structure $s_A$. In website recommender systems, a feasible action $A$ is a set of $K$ websites listed in linear order and the user, faced with such a website list, will check the website one by one from the beginning. In social influence problem, a feasible action is a subset of people in graph structure. In the rest of the paper, we abuse the notation of $A$ to denote the feasible action and also the corresponding underlying set. We define the {\it size} of feasible action $A$ to be the number of base arms in the underlying set of $A$, denoted by $|A|$. We assume all feasible actions considered in this paper have size at most $K$. In many of applications, the expected reward of an action relates not only the underlying set but also the structure, or the order of the base arms. For example in recommender systems, to improve user experience, the items with higher preference will be listed higher.

As a convention, we always use boldface symbols to represent random variables.

At time $t$, feature vectors $x_{t,a} \in \RR^{d \times 1}$ with $\norm{x_{t,a}}_2 \leq 1$ for every base arm $a \in E$ are revealed to the learning agent; the feature vectors combine both the information of the user and the corresponding base arm. \shengyu{Add a standard/typical example (just one sentence) to illustrate the combination?} For example, suppose the user at time $t$ is represented by the feature vector $u_t$ and the feature vector of the base arm $a$ is $v_a$, then we can let $x_{t,a}$ be $(u_t, v_a)$ which combines both the information of the user and the base arm. \shuai{I have added one sentence.} Then the learning agent recommends a feasible action $\bA_t=\{ \ba_{1}^t,...,\ba_{\abs{\bA_t}}^t \} \in \cS$ to the user. Notice that the size of the action is not fixed. Then according to the user behavior and the structure of the action $\bA_t$, we observe the weights of base arms in $\bO_t \subset \bA_t$. For example in website recommender systems, if the user checks from the first recommended website and stops when she finds one interesting or has checked all websites, then the observed base arms are checked websites. 

We use random variable $\bw_{t}(a) \in [0,1]$ as the {\em weight} of base arm $a$ at time $t$. Let $\cH_t$ denote the history before the learning agent chooses action at time $t$. Thus $\cH_t$ contains all information and observations at time $s < t$ and context information at time $t$. Given history $\cH_t$, we assume $\bw_{t,a}$'s are mutually independent and satisfy
\begin{equation} % eq:expectation
	% \label{eq:expectation}
	\EE[\bw_{t,a} | \cH_t] = \theta_{\ast}^{\top} x_{t,a},
\end{equation}
...
We don't require the agent knows how the function $f(A, w)$ is defined. We only assume the agent has access to an oracle $O(A, w)$.

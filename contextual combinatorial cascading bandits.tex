...
Let $\cS \subseteq \cup_{k=1}^K \prod^{k}$ be the set of feasible actions with length no more than $K$. As a convention, we use boldface symbols to represent all random variables.
...
Let $\cH_t$ denote all history before the learning agent chooses action at time $t$; $\cH_t$ contains all information and observations at time $s < t$ and context information at time $t$. Given history $\cH_t$, we assume $\bw_{t,a}$'s are mutually independent and satisfy
...
Because $\cH_t$ denotes all history before time $t$ and context information of time $t$, $\cH_t$ consists of $\{x_s, \bA_s = (\ba_{1}^s,...,\ba_{\abs{\bA_s}}^s), \bO_s, \bw_s(\ba_k^s), k \in[\bO_s], s<t, x_t \}$ and define $\EE_t[\cdot] = \EE[\cdot | \cH_t]$. Here we abuse the notation $\abs{\bA_s}$ to denote the number of base arms in the action $\bA_s$. By Equation \eqref{eq:expectation}, we have 
$$

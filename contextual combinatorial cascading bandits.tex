...
\end{align*}
We also define $p^* = \min_{1 \leq t \leq T} \min_{A \in \cS} ~ p_{t, A}$ to be the minimal probability that an action could have all base arms observed under the 
...
&\qquad \qquad \cdot \left(\sqrt{d\log(\lambda + C_\gamma T/d) + 2\log\left(\frac{1}{\delta}\right)} + \sqrt{\lambda}\right),
\end{split}
\end{equation}
\begin{equation}
\begin{split}

...
From the above lemma, we can bound the regret at time $t$ in terms of the observed base arms, which we would like to estimate in the following. Before that, we need a technical lemma.
...
Now we can give the following estimate about all observed base arms.
...
&\geq \det(\bV_{t-1}) (1 + \sum_{k=1}^{\bO_t} \norm{\gamma_k x_{t,\ba_k^t}}_{\bV_{t-1}^{-1}}^2) \label{eq:detGeq} \\
&\geq \det(\lambda I)\prod_{s=1}^{t}(1 + \sum_{k=1}^{\bO_s} \norm{\gamma_k x_{s,\ba_k^s}}_{\bV_{s-1}^{-1}}^2), \notag
\end{align}
where inequality \eqref{eq:detGeq} is because of Lemma \ref{lem:detTech}.
...
By the above lemma, we need to estimate $\det(\bV_t)$ to bound regret.
...
\end{align}
The inequality \eqref{eq:pfMdeltaEst} is by Lemma \ref{lem:DeltaEsimateWithP*}. The second inequality \eqref{eq:pfMbeta} is because $\bbeta_{t}(\delta)$ is increasing with respect to $t$, derived by definition of $\bbeta_t(\delta)$ and Lemma \ref{lem:detVt}. The third inequality \eqref{eq:pfMmeanEq} is because of mean inequality. The fourth inequality \eqref{eq:pfMsumXi} is because of the definition of $\beta_t(\delta)$ and Lemma \ref{lem:SumXiEstimateInDet}. And the last inequality \eqref{eq:pfMdetEst} is because estimate of $\det(\bV_t)$ by Lemma \ref{lem:detVt}.

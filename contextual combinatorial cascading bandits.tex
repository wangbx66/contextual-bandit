...
\begin{proof} %proof outline of main theorem
	...
	\begin{lemma}
		\CLemmaPrefixExi
	\end{lemma}
	Then we need to bound $f(A, \bU_t)$ and $f_{\wedge}(A, \bU_t)$ which is non-linear and contains discounted parameters $\gamma$. First we have the expected reward function is increasing with respect to $w$ when $\gamma~s$ are decreasing and $\gamma's$ are increasing.
	\begin{lemma}
		\CLemmaIncre
	\end{lemma}
	Then we have an estimate of the expected reward with UCB $\bU_t$.
	\begin{lemma}
		\CLemmaTech
	\end{lemma}
	Next we only need to estimate $\det(\bV_t)$ and those contextual information of observed base arms in $\bV$-norms.
	\begin{lemma}
	...
	\begin{lemma}
		\CLemmaDetVt
	\end{lemma}
	Then it is easy to derive the proof.
\end{proof}
...
\begin{proof}
	...
	\textbf{lem:DeltaEsimateWithP*}\end{align*}
\end{proof}
...
\begin{proof}[ of Theorem \ref{thm:main}]
	...
	Inequality \eqref{eq:pfMsumXi} is because of the definition of $\beta_t(\delta)$ (Eq.\eqref{eq:definebeta}) and Lemma \ref{lem:SumXiEstimateInDet}. 

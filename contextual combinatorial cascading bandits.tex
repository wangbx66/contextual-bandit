...
\wei{I revised the table, including adding a column about general reward, to match the introduction.}
...
\wei{I removed all references to "weight" so far. I think it is a bit unclear and could be confusing.}
...
The work of \cite{combes2015learning} considered a similar recommendation model of cascading bandits with position discounts, where all users are characterized into different classes and each item has a class-specific "reward". 
\wei{So, does that mean \cite{combes2015learning}, comparing to ours, only lacks of contextual
	information? or it our reward function is also more general? It is not very clear to me.}
\shuai{They dealt with discounted 'or' reward. At each time, suppose the user is from class $c$, then recommending her with $K$ items from item-class $h(c)$. We have more general reward function and more general context information.}
...
\wei{please cite the arxiv version below}
...
The assumption of Lipschitz continuity gives an estimate of changes in reward function. To obtain a good estimation of reward function, the position $k$ with large discount value $\gamma_k$ needs a good estimation of $w(a_k)$. This means the positions with large discount $\gamma_k$ are more important to the reward function. For example in website recommendations, the learning agent will receive highest reward if the user selects the first item, so the first position is most important. 

\wei{"large discount is more important" sounds weird. Usually large discount means cheaper and less
	important products. 
Perhaps we should seriously consider not using "position discount". It is a position weight, but
since weight is already used, we should call it something else. Perhaps "position factor"? 	
}
\shuai{What if I change this part by 'large $\gamma_k$'?}
...
\wei{I think just writing $f(A,w_t)$ misses the context part. 
	$f$ as such defined is only a function of action $A$ and expected reward $w$, the subscript $t$
	is part of $w_t$, so is not an effective parameter. 
	We may need to explicitly write context into $f$, such as
	$f(A, w_t, x_t)$. 
	}


\wei{About approximation ratio $\alpha$: Do we have an example with an approximation oracle? 
	If not, we could remove it.
	It is also ok to keep it, but then in Theorems, like Theorem 4.3, we should say 
	$\alpha$-regret.
}

\shuai{There is no experiment with an approximation oracle. But if we want to add some experiment beyond our setting, maybe we can keep it.}	

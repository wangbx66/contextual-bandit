...
\begin{proof}
	...
	\begin{align*}
	\trace(\bV_t) &= \trace(\lambda I) + \sum_{s=1}^t \sum_{k=1}^{\min\{\bO_s,\abs{\bA_s}\}} \gamma^{2k-2} \trace(X_{s,\ba_k^s} X_{s,\ba_k^s}^{\top})\\
	& = d \lambda + \sum_{s=1}^t \sum_{k=1}^{\min\{\bO_s,\abs{\bA_s}\}} \gamma^{2k-2} \norm{X_{s,\ba_k^s}}_2^2\\
	& \leq d \lambda + \sum_{s=1}^t\sum_{k=1}^{K}\gamma^{2k-2}=d\lambda + tC_\gamma.
	\end{align*}
\end{proof}
...
\begin{proof}
	...
	\begin{align*}
	\det(I + \sum_{i=1}^n x_i x_i^{\top}) &= \prod_{j=1}^d (1 + \alpha_j)\geq 1 +\sum_{j=1}^d \alpha_j =1-d + \sum_{i=1}^d (1+\alpha_i) \\

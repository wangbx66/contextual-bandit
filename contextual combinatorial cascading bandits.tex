...
%In the following, we will mainly use notations for disjunctive objective when the two cases are similar.
...
$$
Then, the learning agent update $\bV_t, \bX_t, \bY_t$ in order to get a newer estimate $\hat{\theta}_t$ of $\theta_*$ and new confidence radius $\beta_t(\delta)$. In this part, we use the notation $[A; B]$ to denote the matrix obtained by stack A and B vertically like

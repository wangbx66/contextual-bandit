...
Let 
$$
\beta_{t}(\delta) = \sqrt{\ln(\frac{\det(\bV_{t})}{\det(\lambda I)}) + 2 \ln\left(\frac{1}{\delta}\right)} + \sqrt{\lambda}.
$$
...
\STATE {$\beta_{t}(\delta) \leftarrow \sqrt{\ln(\det(\bV_{t})) + 2 \ln(\frac{1}{\delta})} + \sqrt{\lambda}$}
...
\shuai{The all $\log$'s have base e. I have changed them to be $\ln$.}
...
\shuai{Sorry. It should be $R(t, \bA_t)$ here. I have revised them all.}
\begin{lemma}

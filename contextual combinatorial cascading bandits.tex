...
\begin{theorem}
\label{thm:main}
Suppose $1 = \gamma_1 \geq \gamma_2 \geq \cdots \gamma_{K} \geq c f^*$ for some constant $c > 1$ and $\gamma_k' = 1 - \gamma_k, \gamma'_K \geq \frac{1}{4}f_{\wedge}^{\ast}$. Let regularization parameter satisfy $\lambda \geq C_\gamma$. Then for any $\delta > 0$, with probability at least $1 - \delta$, the cumulative expected regrets of our algorithm, {\em ConComCascade} for both conjunctive and disjunctive objectives, satisfy
\begin{align}
&R(T) = O(\frac{d}{c-1} \log(C_\gamma T) \sqrt{KT}),\\

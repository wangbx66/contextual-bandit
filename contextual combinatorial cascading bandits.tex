...
\begin{equation} % eq:expectation
  ...
  \EE[\bw_{t,a} | \cH_t] = \theta_{\ast}^{\top} x_{t,a},
\end{equation}
where $\theta_{\ast}$ is an unknown $d$-dimensional vector with the assumption that $\norm{\theta_{\ast}}_2 \leq 1$ and $0 \leq \theta_{\ast}^{\top} x_{t,a} \leq 1$ for all $t, a$. Define $w_{t,a} = \theta_{\ast}^{\top} x_{t,a}$ as the expected weight of base arm $a$ at time $t$.
...
$$
	|f(A, w) - f(A, w')| \leq B\sqrt{\sum_{k=1}^{|A|} \gamma_k^2(w(a_k) - w'(a_k))^2},
$$
...
\begin{proof} [of Theorem \ref{thm:main}] %proof outline of main theorem
    ...
    $$
    	R(t, \bA_t) \leq B \sqrt{\sum_{k=1}^{\bO_t} \bbeta_{t-1}(\delta) \| \gamma_k x_{t, \ba_k^t} \|_{\bV_{t-1}}^2}
    $$
...
\begin{align*}
	\br_{t} &= \begin{cases}
		1 - \gamma_{\bO_{t}}  &\text{if } \bw_t(\ba_{\bO_{\wedge, t}}^t) = 0,\\
		1 &\text{if } \bw_t(\ba_{\bO_{\wedge, t}}^t) = 1,
	\end{cases}\\
	&=\begin{cases}
...
\begin{align}
	&f: \cS \times [0,1]^E \to [0,1] \nonumber \\
	&f(A,w) = \sum_{k = 1}^{\abs{A}} \gamma_k' \prod_{i = 1}^{k - 1} w(a_i)(1 - w(a_k)) + \prod_{i=1}^{\abs{A}}w(a_i),
\label{eq:functionfstar}
\end{align}
...
f(A,w) = \sum_{k = 1}^{\abs{A}} \gamma_{k} \prod_{i=1}^{k-1} (1 - w(a_i)) w(a_k),
$$
defined in~Eq.\eqref{eq:OrRewardFunc}, satisfies the properties of monotonicity and Lipschitz continuity.
...
\begin{lemma}[Lipschitz continuity]
	...
	$$
	|f(A, w) - f(A, w')| \leq \sqrt{K}\sqrt{\sum_{k=1}^{|A|} \gamma_k^2 |w(a_k) - w'(a_k)|^2},
	$$
	where $A = (a_1, \ldots, a_{|A|})$.
\end{lemma}
\begin{proof}
	...
	\begin{align*}
		&f(A^{(m+1)}, w')\\
		=&f(A^{(m)}, w') + \gamma_{m+1}\prod_{k=1}^m(1 - w'(k)) w'(m+1)\\
		\leq &f(A^{(m)}, w') +  \gamma_{m+1} \prod_{k=1}^m(1 - w(k)) (w(m+1) + v(m+1))\\
		\leq &f(A^{(m)}, w) + \sum_{k=1}^m \gamma_k v(k) \\
		&\qquad + \gamma_{m+1} \prod_{k=1}^m(1 - w(k)) w(m+1) + \gamma_{m+1} v(m+1)\\
		=& f(A^{(m+1)}, w) + \sum_{k=1}^{m+1} \gamma_k v(k).
	\end{align*}
	So it holds for $m+1$.
...
In this section, we prove Theorem \ref{thm:and}. Recall the expected reward function is defined as

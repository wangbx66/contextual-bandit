...
Our proposed algorithm, ConComCascade, is described in Algorithm \ref{alg:ConComCascade}. 
...
$$
denote the best expected reward in all time under the conjunctive objective and let $p_{t, \bA_t}$ (resp., $p_{\wedge, t, \bA_t}$) are probabilities that all base arms of $A = (a_1, \ldots, a_{\abs{A}})$ have been observed under disjunctive objective(resp., conjunctive), that is 
\begin{align*}
&p_{t, A} = \prod_{k=1}^{\abs{A}-1} (1 - \theta_{\ast}^{\top} x_{t, a_k^t}),\\
&p_{\wedge, t, A} = \prod_{k=1}^{\abs{A}-1} (\theta_{\ast}^{\top} x_{t, a_k^t}).
\end{align*}
We also define $p^* = \min_{1 \leq t \leq T} \min_{A \in \cS} ~ p_{t, A}$ to be the minimal probability that an action could have all base arms observed under disjunctive case. Here is our main theorem.
\begin{theorem}
\label{thm:main}
...
$$
Let regularization parameter satisfy $\lambda \geq C_\gamma$. Then for any $\delta > 0$, with probability at least $1 - \delta$, the cumulative expected regrets of our algorithm, {\em ConComCascade} for both conjunctive and disjunctive objectives, satisfy
\begin{equation}
\begin{split}
&R(T) = \frac{\sqrt{2}}{p^*} (\sqrt{d\log(\lambda + C_\gamma T/d) + 2\log(\frac{1}{\delta})} + \sqrt{\lambda}) \\
&\qquad \qquad \cdot \sqrt{TKd\log(\lambda + C_\gamma T/d)},
\end{split}
\end{equation}
\begin{equation}
\begin{split}
&R_{\wedge}(T) = \frac{\sqrt{128}}{f_{\wedge}^{\ast}} (\sqrt{d\log(\lambda + C_\gamma T/d) + 2\log(\frac{1}{\delta})} + \sqrt{\lambda})\\
&\qquad \qquad \cdot \sqrt{TKd\log(\lambda + C_\gamma T/d)}.
\end{split}
\end{equation}
where

...
def contextual_movielens_rng(L=None, portion=0.5, d=15, K=6, h=None, gamma=0.95, rgamma=0.95, sort=False, disj=False):
    cox = []
    coy = []
    history = {}
    movies = {}
    ...
    s = ucb_settings(L=L, d=d, K=K, gamma=gamma, rgamma=rgamma, sort=sort, disj=disj, users=eligable_users, arms=selected_movies, ctrh=history, U=U, V=V, A=A.astype(np.int32))
    logger.info(s)
    return s

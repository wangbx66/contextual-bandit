...
def flowtest_monkey(T, kw):
    ...
    exploit1, explore1 = contextual_cascading_sherry(*contextual_monkey(s, cascade=True), T=T)
    exploit2, explore2 = contextual_cascading_monkey(*contextual_monkey(s, cascade=True), T=T)
    exploit3, explore3 = contextual_full_lijing(*contextual_monkey(s, cascade=False), T=T)
    exploit4, explore4 = absolute_cascading_ucb(*contextual_monkey(s, cascade=True), T=T)
    exploit5, explore5 = absolute_cascading_gammaucb(*contextual_monkey(s, cascade=True), T=T)
...
def flowtest_movielens(T, kw):
    ...
    exploit1, explore1 = contextual_cascading_monkey(*contextual_movielens(s, cascade=True), T=T)
    exploit2, explore2 = contextual_cascading_sherry(*contextual_movielens(s, cascade=True), T=T)
    exploit3, explore3 = contextual_full_lijing(*contextual_movielens(s, cascade=False), T=T)
    exploit4, explore4 = absolute_cascading_ucb(*contextual_movielens(s, cascade=True), T=T)
    plt.plot(range(T), exploit1, 'r--', range(T), exploit2, 'r--', range(T), exploit3, 'b--', range(T), exploit4, 'b--')
...
def flowtest_isp(T, kw):
    ...
    exploit1, explore1 = contextual_cascading_monkey(*contextual_isp(s, cascade=True), T=T)
    exploit2, explore2 = contextual_cascading_sherry(*contextual_isp(s, cascade=True), T=T)
    exploit3, explore3 = contextual_full_lijing(*contextual_isp(s, cascade=False), T=T)
    exploit4, explore4 = absolute_cascading_ucb(*contextual_isp(s, cascade=True), T=T)
    plt.plot(range(T), exploit1, 'r--', range(T), exploit2, 'r--', range(T), exploit3, 'b--', range(T), exploit4, 'b--')

#kw = {'L':20, 'd':10, 'h':0.35, 'K':4, 'gamma':0.95, 'eps':0.1, 'v':0.35, 'disj':True}
kw = {'L':20, 'd':10, 'h':0.75, 'K':4, 'gamma':0.95, 'rgamma':0.95, 'eps':0.1, 'v':0.35, 'sort': False, 'disj':False}
